    ...
    <section id="live-demo" className="mx-auto max-w-[1100px] px-4 py-7 sm:px-5 md:px-12 md:py-10">
      <div className="grid items-start gap-7 lg:grid-cols-[0.4fr_0.6fr] lg:gap-14">
        <div className="max-w-[420px]">
          ...
          <h2 className="mb-4 text-balance text-[24px] font-medium leading-tight tracking-[-0.5px] text-[#0F1117] sm:text-[34px] md:text-[36px]">
            Keep using ChatGPT. stemLM adds the structure AI skips.
          </h2>
          <p className="max-w-[520px] text-[16px] leading-[1.7] text-[#64748B]">
        ...
        {steps.map((step, index) => {
          ...
          return (
            <div key={step.number} className="relative flex items-start gap-4 sm:gap-5">
              {!isLast ? (
              ...
              <div className={`flex-1 ${isLast ? "pb-0" : "pb-7 sm:pb-8"}`}>
                <h3 className="mb-1 text-[15px] font-medium text-[#0F1117] sm:text-[16px]">{step.title}</h3>
                <p className="mb-3 text-[13px] leading-[1.65] text-[#64748B] sm:text-[14px] sm:leading-[1.6]">{step.body}</p>
