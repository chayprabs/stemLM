...
export function Hero({ onOpenModal }: HeroProps) {
  ...
  const replaySeconds = Math.max(1, Math.ceil(timerWidth / 16.67));
  ...
  useEffect(() => {
    ...
    timerIntervalRef.current = setInterval(() => {
      nextWidth = Math.max(0, nextWidth - 100 / 60);
      setTimerWidth(nextWidth);
  ...
  return (
    <section className="mx-auto grid max-w-[1200px] grid-cols-1 items-start gap-9 px-4 pb-8 pt-10 sm:px-5 md:gap-14 lg:grid-cols-[55fr_45fr] lg:px-12 lg:pb-12 lg:pt-16">
      ...
      <div className="mx-auto w-full max-w-[520px]">
        ...
        >
          ...
          >
            <div className="flex h-full flex-col">
              ...
              <div style={{ width: "100%", height: "2px", background: "#E2E8F0", flexShrink: 0 }}>
                <div
                  style={{
                    width: `${timerWidth}%`,
                    height: "2px",
        ...
        <div className="mt-2.5 text-center text-[11px] text-[#8A8A9A]">{statusText}</div>
      </div>
    </section>
  );
}
